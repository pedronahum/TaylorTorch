...
func groupnorm_forward_manual_nchw() throws {
  ...
  var gn = GroupNorm(
    numGroups: 2, numChannels: 4, epsilon: 1e-5, dataFormat: .nchw, dtype: .float64)
  gn.weight = Tensor(array: [1.5, 0.5, -1.0, 2.0], shape: [4], dtype: .float64)
  gn.bias = Tensor(array: [0.1, -0.2, 0.0, 0.25], shape: [4], dtype: .float64)

  let spatial = x.shape.dropFirst(2).reduce(1, *)
  let channelsPerGroup = x.shape[1] / gn.numGroups
  let grouped = x.reshaped([x.shape[0], gn.numGroups, channelsPerGroup * spatial])
  ...
  let norm = centered.dividing(var_.adding(1e-5).sqrt())
  let normNCHW = norm.reshaped(x.shape)

  let gamma = gn.weight.reshaped([1, 4, 1, 1])
  let beta = gn.bias.reshaped([1, 4, 1, 1])
...
func groupnorm_nhwc_parity() throws {
  ...
  var nchw = GroupNorm(numGroups: 2, numChannels: 4, dataFormat: .nchw, dtype: .float64)
  var nhwc = nchw
  nchw.weight = Tensor(array: [1.5, 0.5, -1.0, 2.0], shape: [4], dtype: .float64)
  nchw.bias = Tensor(array: [0.1, -0.2, 0.0, 0.25], shape: [4], dtype: .float64)
  nhwc.weight = nchw.weight
  nhwc.bias = nchw.bias
...
func groupnorm_parameter_grads() throws {
  ...
  var gn = GroupNorm(numGroups: 2, numChannels: 4, dtype: .float64)
  gn.weight = Tensor.ones(shape: [4], dtype: .float64)
  gn.bias = Tensor.zeros(shape: [4], dtype: .float64)
  ...
  let centered = grouped.subtracting(mean)
  let var_ = centered.multiplying(centered).mean(dim: 2, keepdim: true)
  let norm = centered.dividing(var_.adding(1e-5).sqrt()).reshaped(x.shape)

  let expectedDgamma = norm.sum(dim: 0).sum(dim: 1).sum(dim: 1)
  let nhw = Double(x.shape[0] * x.shape[2] * x.shape[3])
  var expectedDbeta = Tensor.full(nhw, shape: [x.shape[1]], device: g.bias.device)
  if let dtype = g.bias.dtype, expectedDbeta.dtype != dtype {
    expectedDbeta = expectedDbeta.to(dtype: dtype)
  }

  #expect(g.weight.isClose(to: expectedDgamma, rtol: 1e-9, atol: 1e-9, equalNan: false))
  #expect(g.bias.isClose(to: expectedDbeta, rtol: 1e-9, atol: 1e-9, equalNan: false))
}
...
func groupnorm_input_gradients_match_fd() throws {
  ...
  var gn = GroupNorm(numGroups: 2, numChannels: 4, epsilon: 1e-5, dataFormat: .nchw, dtype: .float64)
  gn.weight = Tensor(array: [1.3, -0.7, 0.5, 2.0], shape: [4], dtype: .float64)
  gn.bias = Tensor(array: [0.1, 0.0, -0.2, 0.25], shape: [4], dtype: .float64)
...
func groupnorm_nhwc_gradients_parity() throws {
  ...
  var nchw = GroupNorm(numGroups: 2, numChannels: 4, epsilon: 1e-5, dataFormat: .nchw, dtype: .float64)
  nchw.weight = Tensor(array: [1.25, -0.4, 0.75, 1.5], shape: [4], dtype: .float64)
  nchw.bias = Tensor(array: [0.0, 0.2, -0.3, 0.5], shape: [4], dtype: .float64)
  ...
  #expect(gradsNHWC.weight.isClose(to: gradsNCHW.weight, rtol: 1e-12, atol: 1e-12, equalNan: false))
  #expect(gradsNHWC.bias.isClose(to: gradsNCHW.bias, rtol: 1e-12, atol: 1e-12, equalNan: false))
}
