...
func permutedGradientMatchesAnalytic() throws {
  ...
  let upstream = Tensor.arange(Double(0), to: Double(24), step: 1, dtype: .float64).reshaped([4, 2, 3])
  let grad = pullback(upstream)
...
func shapeChangingGradientsRestoreOriginalShape() throws {
  ...
  #expect(reshapedValue.isClose(to: input.reshaped([3, 4]), rtol: 1e-6, atol: 1e-6, equalNan: false))
  let reshapedUpstream = Tensor.arange(Double(0), to: Double(12), step: 1, dtype: .float64).reshaped([3, 4])
  ...
  #expect(flattenedValue.isClose(to: input.flattened(startDim: 1, endDim: 2), rtol: 1e-6, atol: 1e-6, equalNan: false))
  let flattenedUpstream = Tensor.arange(Double(0), to: Double(12), step: 1, dtype: .float64).reshaped([2, 6])
...
func squeezeUnsqueezeGradientsRestoreAxes() throws {
  let squeezedInput = Tensor.arange(Double(0), to: Double(6), step: 1, dtype: .float64).reshaped([1, 2, 1, 3])
  let (squeezedValue, squeezedPullback) = valueWithPullback(at: squeezedInput) { tensor in
    tensor.squeezed()
  }
  #expect(squeezedValue.isClose(to: squeezedInput.squeezed(), rtol: 1e-6, atol: 1e-6, equalNan: false))
  ...
  #expect(squeezedDimValue.isClose(to: squeezedDimInput.squeezed(dim: 1), rtol: 1e-6, atol: 1e-6, equalNan: false))
  ...
  #expect(unsqueezedValue.isClose(to: unsqueezedInput.unsqueezed(dim: 1), rtol: 1e-6, atol: 1e-6, equalNan: false))
