...
public extension Tensor {
  ...
  static func rand(shape: [Int], dtype: DType = .float32, device: Device = .cpu) -> Tensor {
  ...
  ///   - dtype: Element dtype for the output (defaults to `.float32`).
  ///   - device: Execution device on which to allocate storage.
  static func randn(shape: [Int], dtype: DType = .float32, device: Device = .cpu) -> Tensor {
    var s64 = shape.map(Int64.init)
  ...
  static func arange<T: TorchInteger & TorchArithmetic & ExpressibleByIntegerLiteral>(
    _ start: T, to end: T, step: T, dtype: DType? = nil, device: Device = .cpu
  ) -> Tensor {
    let dt = (dtype ?? T.torchDType)._c10
    return Tensor(TTSTensor.arange(start._cxxScalar, end._cxxScalar, step._cxxScalar, dt, device._c10))
  }
  ...
  ///   - dtype: Optional override for the tensor dtype; defaults to the scalar type of `T`.
  ///   - device: Execution device for the resulting tensor.
  static func arange<T: TorchFloating & TorchArithmetic & ExpressibleByFloatLiteral>(
    _ start: T, to end: T, step: T, dtype: DType? = nil, device: Device = .cpu
  ) -> Tensor {
    let dt = (dtype ?? T.torchDType)._c10
    return Tensor(TTSTensor.arange(start._cxxScalar, end._cxxScalar, step._cxxScalar, dt, device._c10))
  ...
  ///   - dtype: Element dtype for the output (defaults to `.float32`).
  ///   - device: Execution device on which to allocate storage.
  static func linspace<T: BinaryFloatingPoint>(
