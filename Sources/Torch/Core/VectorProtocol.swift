...
extension VectorProtocol {
  internal static func visitChildren(
    ...
    let ok = _forEachFieldWithKeyPath(
      of: Self.self,
      body: { _, kp in
        let valueType = type(of: kp).valueType
        guard let valueType = valueType as? _VectorProtocol.Type else {
          fatalError("VectorProtocol member is not _VectorProtocol: \(valueType)")
        }
        body(kp, valueType)
        return true
      })
    precondition(ok, "Unreflectable member of \(Self.self) while implementing VectorProtocol.")
  }
}
...
extension Tensor: _VectorProtocol {
...
where Element: Differentiable & VectorProtocol {
  public mutating func add(_ x: Float) {
    for i in self.base.indices { self.base[i].add(x) }
  }
  public mutating func subtract(_ x: Float) {
    for i in self.base.indices { self.base[i].subtract(x) }
  }
  public mutating func scale(by scalar: Float) {
    for i in self.base.indices { self.base[i].scale(by: scalar) }
  }
}
